...
export class TokenreportsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['tokenNo', 'mrnNo', 'ptFstName', 'appointmentTime', 'checkInTime', 'checkInMode', 'deptEngName', 'serviceEngName'];
...
defaultServieData:IService={
  ...
  constructor(private datePipe: DatePipe ,private fb: FormBuilder, private tokenservice:TokenReportService, private appConfig: AppConfig, private router: Router, private alertMessage: AlertMessageService, private translate: TranslateService) {
    ...
    if (this._tokenInfo && tokenData) {console.log('ifffff');
  ...
  togglePerOne(type){   
    if(type =='floor') {
      ...
      else if(this.tokenForm.value.floor.length==0){
        this.filterFloorGroup=[];
        this.filterDeptGroup=[];
        this.tokenForm.get('department').setValue([]);      
        this.tokenForm.get('service').setValue([]);      
  ...
  loadServices(departments){ console.log('departments',departments);
    ...
    if(departments){
      ...
      if(servicedata){
        ...
        console.log('count=>',count,this.tokenForm.value.department);
  ...
  selectedModule(services){console.log('services =>',services);
  ...
  selectPeriod(value){
    if(value == 0){
      this.tokenForm.get('fromDate').clearValidators();
      this.tokenForm.get('fromDate').updateValueAndValidity();
      this.tokenForm.get('toDate').clearValidators();
      this.tokenForm.get('toDate').updateValueAndValidity();
  ...
  getReports(){
    ...
    let selDeptList: Array<number> = this.tokenForm.value.department;
    selDeptList = this.getFields(selDeptList, "deptId");
 
    console.log('selDeptList ',selDeptList);
    
    let selServList:Array<number> = this.tokenForm.value.service;
    ...
    this.tokenForm.value.service

    let searchData = {
      "endDate": this.toDate,
      "orgId": this.orgId,   
      "selectionType": this.tokenForm.value.dateRange,  
      "services": selServList,
      "startDate": this.fromDate,
      "tokenNo" : this.tokenForm.value.tokennumber
    };
  ...
  ExportTOExcel() {
    this.list  = this.reportsList;
    this.list.forEach(element => {
      element.ptFstName = element.ptFstName+ ' ' + element.ptLstName;
      element.appointmentTime = this.datePipe.transform(element.appointmentTime, 'dd/MM/yyyy, HH:mm')
      element.checkInTime = this.datePipe.transform(element.checkInTime, 'dd/MM/yyyy, HH:mm')
    });
    console.log("List-Excel ==>", this.list);
  


    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.list.map(data=> 
  ...
  ExportTOCsv() {
    ...
    console.log("List-Csv ==>", this.list);
    var options = {
      noDownload: false,
      headers:this.displayedColumns,
    };
    new Angular5Csv(this.list.map(data=> _.pick(data, this.displayedColumns)), 'Service_Reports',options);
  }
